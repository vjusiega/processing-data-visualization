...
function draw() {
  ...
  for(i = 0; i < hourSquares.length; i++){
    rect(hourSquares[i].x - move, hourSquares[i].y - move, hourSquares[i].w - move, hourSquares[i].h - move);
  }
  
  strokeWeight(3);
  for(j = 0; j < minuteSquares.length; j++){
    rect(minuteSquares[j].x - move, minuteSquares[j].y - move, minuteSquares[j].w - move, minuteSquares[j].h - move);
  }
  
  strokeWeight(1);
  for(k = 0; k < secondSquares.length; k++){
    rect(secondSquares[k].x - move, secondSquares[k].y - move, secondSquares[k].w - move, secondSquares[k].h - move);
  }
